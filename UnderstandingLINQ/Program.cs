    ...
    class Program
    {
        static void Main(string[] args)
        {
            List<Car> myCars = new List<Car>()
            {
                ...
                new Car (){VIN= "E5", Make = "BMW", Model= "55i", StickerPrice = 57000, Year=2009}
            };
            // LINQ query
            /*var bmws = from car in myCars
                       ...
                       && car.Year == 2010
                       select car;*/
            /* var orderedCars = from car in myCars orderby
                               car.Year descending select car;*/
            //LINQ method
            // var bmws = myCars.Where(p => p.Make == "BMW" && p.Year ==2010);

            // var orderedCars = myCars.OrderBy(p => p.Year);
            /*
            var firstBMW = myCars.OrderBy(p=>p.Year).First(p => p.Make == "BMW");
